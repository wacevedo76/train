...
# Create a directory to store log files with erros.
...
for file in $(ls /var/log/*.log); do
  ...
  if [[ $? -eq 0 ]]; then
    ...
    # Create the new file location variable with the directory and basname of the file
    file_new_location="${ERROR_DIRECTORY}$(basname=${file})"
