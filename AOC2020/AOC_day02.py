...
def CheckPasswordValidity(dataset):
    # Create a var that holds an in representing the total number of valid passwords
    validPasswords = 0
    
    # itertate over each set of password validity data
    # example dataset ['13-71' , 's' 'ssssssssssssssss']
    for passwordSet in dataset:
        # make variable to keep track of letter occurences
        letter_occurances = 0

        # set necessary set data
        passwordRange = passwordSet[0].split('-')
        passwordTestChar = passwordSet[1]
        passwordString = passwordSet[2]
    
        # itterate through entire sting
        for char in passwordString:
            #record the number of times a letter occurse in the loop
            if char == passwordTestChar:
                letter_occurances += 1

        # Validate that the amount of occurences fall between the alloted constraint
        if letter_occurances >= int(passwordRange[0]) and letter_occurances <= int(passwordRange[1]):
            # increment valid passwords
            validPasswords += 1

    return validPasswords

def CheckPasswordValidity2(dataset):
    # Create a var that holds an in representing the total number of valid passwords
    validPasswords = 0
    ...
    for passwordSet in dataset:
        # set necessary set data
        p1, p2 = passwordSet[0].split('-')
        passwordTestChar = passwordSet[1]
        psswrdStr= passwordSet[2]

        # assign both conditions (booleans) to variables
        charEither = passwordTestChar == psswrdStr[int(p1) - 1] or passwordTestChar == psswrdStr[int(p2) - 1]
        charBoth =  passwordTestChar == psswrdStr[int(p1) - 1] and passwordTestChar == psswrdStr[int(p2) - 1]

        # test string to check values
        # print(f"{psswrdStr} | {passwordTestChar} | {str(p1)}:{str(p2)} | {psswrdStr[int(p1) - 1]}:{psswrdStr[int(p2) - 1]} | {charEither}:{not charBoth}")

        # if both values are true (charEither and NOT charBoth) increment validPasswords
        if charEither and not charBoth:
            validPasswords += 1
    
    return validPasswords

print(str(CheckPasswordValidity2(dataset)))
